...
int main(int argc, char *argv[]) {
  ...
  // [BUG FIXED] Command Injection(only allow .png file as a output name) ->
  // snprintf
   ...
   */
  // [BUG FIXED] - Unchecked system call returning code
  if (fflush(stdout)) {
    fprintf(stderr, "Cannot flush stdout\n");
    return 1;
  }

  //[BUG FIXED] - Command Injection / Unchecked system call( system(command) )
  // TODO: report for unchecked system call and cmd inj

  struct stat file_stat;
  if (stat(output_name, &file_stat)) {
