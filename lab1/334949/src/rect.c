...
int main(int argc, char *argv[]) {
  ...
  unsigned top_left_x = strtol(argv[3], &end_ptr, 10);
  unsigned top_left_y = strtol(argv[4], &end_ptr, 10);

  unsigned bottom_right_x = strtol(argv[5], &end_ptr, 10);
  unsigned bottom_right_y = strtol(argv[6], &end_ptr, 10);

  /* Invalid color code will set the color to black.
   * If it worked for Henry Ford, it will work for us.
   */

  // [BUG FIXED] -> Type Error (end_ptr -> &end_ptr) (need to give it a
  // reference to pointer instead of a pointer) [BUG FIXED] -> Unchecked system
  // call returning code
  ...
  //[BUG FIXED] Iteration error / Heap Buffer Overflow -> i, j
  //
  unsigned i = 0;
  unsigned j = 0;
  while (i < height) {
