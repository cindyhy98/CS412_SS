...
int main(int argc, char *argv[]) {
  ...
  // [BUG FIXED] - Heap Overflow (not divisible index)
  if ((width % square_width) != 0 || (height % square_width) != 0) {
    goto error_squaresize;
  }
  ...
  if (!img->px) {
    //[BUG FIXED] -> Temporal memory allocation -> double free
    // free(img);
    free(img->px);
    goto error_img;
  }
...
error:
  ...
  return 1;
error_squaresize:
  printf("square_width is not divisible by either height or width\n");
  return 1;
error_px:
  free(img->px);
error_img:
